...
export const PostItem: React.FC<PostItemProps> = ({
  ...
  try {
    if (wasLiked) {
      ...
      // ←←← THIS WAS MISSING
      if (post.user_id !== currentUserId) { // don't notify yourself
        await supabase.from('notifications').insert({
          recipient_id: post.user_id,   // post owner
          actor_id: currentUserId,      // who liked it
          type: 'like',
          entity_id: post.id,
        });
      }
    ...
    onLikeToggle(post); // your existing UI update
  } catch (err) {
  ...
  e.preventDefault();
  if (!newCommentText.trim() || !currentUserId || isPostingComment) return;
  ...
  try {
    ...
    if (post.user_id !== currentUserId) {
      await supabase.from('notifications').insert({
        ...
        type: newComment.parent_id ? 'reply' : 'comment', // supports both
        entity_id: post.id,
      });
    }
      ...
      <div className="border-b border-[rgb(var(--color-border))] p-4 hover:bg-[rgb(var(--color-surface-hover))] transition bg-[rgb(var(--color-surface))]">
        ...
        <div className="flex gap-4 items-start">
          <button onClick={() => onNavigateToProfile(post.user_id)} className="flex-shrink-0 relative">
            <img src={post.profiles?.avatar_url || `https://muktoverse.vercel.app/asset/images/disorder_favicon.png`} className="w-12 h-12 rounded-full hover:opacity-80 transition" alt="Avatar" />
              ...
              {likersList.length === 0 ? <p className="text-center text-[rgb(var(--color-text-secondary))]">No likes yet.</p> : likersList.map((liker, idx) => (
                <div key={`${liker.user_id}-${idx}`} className="flex items-center justify-between">
                  <div className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigateToProfile(liker.user_id)}>
                    <img src={liker.profiles?.avatar_url || `https://muktoverse.vercel.app/asset/images/disorder_favicon.png`} className="w-10 h-10 rounded-full" alt="Avatar" />
                    <div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
               {commentsList.length === 0 ? <div className="h-full flex items-center justify-center text-[rgb(var(--color-text-secondary))]">No comments yet.</div> : commentsList.map((comment) => (
                 <div key={comment.id} className="flex gap-3">
                   <img src={comment.profiles?.avatar_url || `https://muktoverse.vercel.app/asset/images/disorder_favicon.png`} className="w-9 h-9 rounded-full cursor-pointer flex-shrink-0" alt="Avatar" onClick={() => onNavigateToProfile(comment.user_id)} />
