...
CREATE TABLE public.blacklisted_media (
  hash_value text NOT NULL,
  content_type text,
  added_at timestamp with time zone DEFAULT now(),
  severity_level text DEFAULT 'critical'::text,
  CONSTRAINT blacklisted_media_pkey PRIMARY KEY (hash_value)
);
...
CREATE TABLE public.security_audit_logs (
  ...
  CONSTRAINT security_audit_logs_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
